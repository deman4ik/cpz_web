...
export const formatVariables = (item, buttonType: string, displayType?: string) => {
  const type = displayType ? displayType === 'signals' ?
    (item.isSubscribed ? 'unsubscribe' : 'subscribe') :
    (!item.user_robots.status ? 'create' : statusTypes[item.user_robots.status]) : buttonType;
  return (
    { variables: {
      cache: item.cache,
      robot: { id: item.id, name: item.name, userRobotId: item.user_robots.id },
      subs: { volume: item.volume, exchange: item.exchange, asset: item.asset, currency: item.currency },
      type,
      isVisible: true } }
  );
};
