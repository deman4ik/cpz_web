...
import { formatDate, moneyFormat } from '../../../config/utils';
...
export const getFormatDataSignals = (positions: any) => positions.reduce((acc, position) => {
  ...
  if (item) {
    ...
    if (!findAsset) {
      ...
      findAsset.volume = Math.floor((findAsset.volume + (robot.direction === 'short' ? -1 : 1) * robot.volume) * 1000) / 1000;
    }
  } else {
    obj.assets = [ asset ];
  }
...
export const getFormatDataRobots = (positions: any) => positions.reduce((acc, position) => {
