...
const propsToMoneyFormat = obj =>
  Object.keys(obj).reduce(
    (acc, k) => ({ ...acc, [k]: `${moneyFormat(obj[k])} $` }),
    {}
  );
const propsToPercent = obj =>
  Object.keys(obj).reduce((acc, k) => ({ ...acc, [k]: `${obj[k]} %` }), {});
const propsSimple = obj =>
  Object.keys(obj).reduce((acc, k) => ({ ...acc, [k]: obj[k] }), {});
const propsToNull = obj =>
  Object.keys(obj).reduce((acc, k) => ({ ...acc, [k]: obj[k] || '-' }), {});
  ...
  (!robotStatistic
    ...
    : {
      profit: [
        ...
        {
          title: 'Maximum Drawdawn',
          key: 'maxDrawdown',
          ...propsToMoneyFormat(robotStatistic.maxDrawdown)
        },
        {
          title: 'Maximum Drawdawn Date',
          key: 'maxDrawdownDate',
...
export const getCardTitle = (item, subtitle) =>
  `${item.title}${
    item.key === 'avgBarsHeld' && subtitle !== 'profit'
      ? ` ${capitalize(subtitle)}`
      : ''
  }`;
