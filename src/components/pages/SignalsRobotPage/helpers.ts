...
export const subscribeAt = (robotData: any) => (
...
export const getFormatDataClosedPositions = (dataClosedPositions, isUserSignals, volume) => (
  dataClosedPositions.robot_positions.map(item => {
    ...
    if (isUserSignals) {
      const profit = (exit_price - entry_price) * volume *
        (direction === 'short' ? -1 : 1);
...
const getEntryMarker = (position_entry, candleRobot, asset) => {
  ...
  const volume = `${candleRobot.user_signals.length ? candleRobot.user_signals[0].volume : position_entry.volume} ${asset}`;
...
const getExitMarker = (position_exit, candleRobot, asset) => {
  ...
  const volume = `${candleRobot.user_signals.length ? candleRobot.user_signals[0].volume : position_exit.volume} ${asset}`;
  const profit = candleRobot.user_signals.length ? candleRobot.user_signals[0].volume * balance : position_exit.profit;
  return {
    ...
    id: `${id}_${1}`,
    code,
    action: capitalize(exit_action).split(/(?=[A-Z])/).join(' '),
    profit,
    volume,
    status,
    exit: true
...
export const getFormatData = (data, asset) => {
  ...
  return data.candles.reduceRight((acc, item) => {
...
export const getFormatUpdateData = (data, asset) => {
  let candles = { time: null, open: null, high: null, low: null, close: null, volume: null };
  const markers = [];
  if (!data || !data.candles.length) return { candles, markers };
  let canAddPosition = false;
...
export const getAlerts = (signals) => (Object.keys(signals.alerts).length
  ? Object.keys(signals.alerts).map(item => {
    const colorItem = (signals.alerts[item].action === 'short' || signals.alerts[item].action === 'closeLong') ? color.negative : color.positive;
    const axisLabelVisible = true;
    const alertItem = { ...signals.alerts[item], volume: signals.volume, code: signals.code, axisLabelVisible, color: colorItem };
    return alertItem;
  })
  : []);
...
export const createVariable = (robotData, type) => {
  ...
  return ({
    variables: {
      ...
      subs: { volume, exchange: robot.exchange, asset: robot.asset, currency: robot.currency },
      type
    }
  });
};
