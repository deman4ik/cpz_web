...
export const useFetchPositionData = (isUserSignals, userSignals, robot) => {
  ...
  const volume = useMemo(() => (
    isUserSignals ? userSignals.volume : 0
  ), [ userSignals ]);
  ...
  const formatSignals = useMemo(() => (
    (!loadingOpenSignals && dataSignals && dataSignals.robot_positions.length && Object.keys(dataSignals.robot_positions[0].alerts).length)
