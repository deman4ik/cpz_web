...
export const createVariable = (robotData, type) => (
  robotData.user_robots
    ? { variables: {
      ...
      robot: { id: robotData.robot.id, name: robotData.robot.name, userRobotId: robotData.user_robots.id },
      subs: { volume: robotData.user_robots.settings.volume, exchange: robotData.robot.exchange, asset: robotData.robot.asset, currency: robotData.robot.currency },
      type
    } }
    : { variables: {
      cache: {
...
export const getFormatSignals = (signals) => Object.keys(signals).reduce((acc, item) => [ ...acc, {
  price: signals[item].price,
  color: (signals[item].action === 'short' || signals[item].action === 'closeLong') ? color.negative : color.positive,
  axisLabelVisible: true
} ], []);

export const activeDays = (robotData: any) => (robotData.robot.active ? dayjs.utc(robotData.robot.active).fromNow(true) : null);
