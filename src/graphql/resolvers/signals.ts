...
export const unsubscribe = (_root: any, variables: any, context: any) => {
  ...
  if (isExistSignals) {
    ...
    context.cache.writeQuery({
      query: USER_SIGNALS,
      data: {
        signals: dataSignals.signals.filter(
          el => el.robot.id !== variables.cache.id
        )
      }
    });
  }
  if (
    variables.cache.tableName === 'robots' ||
    variables.cache.tableName === 'charts'
  ) {
...
export const subscribe = (_root: any, variables: any, context: any) => {
  if (variables.type === 'edit') {
    ...
    ).find(el => el.indexOf('v_robots_stats_signals') === 0);
