...
export const unsubscribe = (_root: any, variables: any, context: any) => {
  const isExistSignals = Object.keys(context.cache.data.data.ROOT_QUERY).find(el => el.indexOf('user_signals_robots') === 0);
  if (isExistSignals) {
  ...
  if (variables.cache.tableName === 'robots' || variables.cache.tableName === 'charts') {
    const idRobots = context.getCacheKey({ __typename: 'robots', id: variables.cache.id });
    ...
    if (variables.cache.tableName === 'charts') {
      ...
      const dataChart = dataCandles.candles.map(item =>
        ({ ...item, robot: { ...item.robot, user_signals: [] } }));
      context.cache.writeQuery({
        query: ROBOT_POSITION_WITH_CANDLE(timeframe),
        variables: { limit, robotId },
        data: {
...
export const subscribe = (_root: any, variables: any, context: any) => {
  if (variables.type === 'edit') {
    if (variables.cache.tableName === 'robots' || variables.cache.tableName === 'charts') {
      ...
      `;
      const row = context.cache.readFragment({ fragment: fragmentRobots, id: idRobots });
      const row_user_signals = row.user_signals.map(rowItem => (
        { ...rowItem, volume: variables.volume }
      ));
      ...
      if (variables.cache.tableName === 'charts') {
        ...
        const dataChart = dataCandles.candles.map(item => {
          const user_signals = item.robot.user_signals.map(robotItem => (
            { ...robotItem, volume: variables.volume }
          ));
    ...
    const isExistRobots = Object.keys(context.cache.data.data.ROOT_QUERY).find(el => el.indexOf('robots_info_user_signals') === 0);
    if (isExistRobots) {
      const idRobots = context.getCacheKey({ __typename: 'robots', id: variables.cache.id });
      const fragmentRobots = gql`
      ...
      context.cache.writeFragment({ fragment: fragmentRobots, id: idRobots, data: { ...row, user_signals: [ userItem ] } });
    }
    const isExistVRobotsStats = Object.keys(context.cache.data.data.ROOT_QUERY).find(el => el.indexOf('v_robots_stats_signals') === 0);
    ...
    if (isExistSignals) {
      ...
      `;
      const item = {
        robot: context.cache.readFragment({ fragment: fragmentRobots, id: idRobots }),
        __typename: 'user_signals'
      };
      context.cache.writeQuery({
        query: USER_SIGNALS,
