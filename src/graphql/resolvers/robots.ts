...
import {
  USER_ROBOTS,
  GET_ROBOTS_BY_STATS,
  GET_ROBOT_INFO_USER_ROBOTS
} from '../robots/queries';

export const deleteRobot = (_root: any, variables: any, context: any) => {
  const isExistRobotsStats = Object.keys(
    context.cache.data.data.ROOT_QUERY
  ).find(el => el.indexOf('v_robots_stats_robots') === 0);
  ...
  const isExistUserRobots = Object.keys(
    context.cache.data.data.ROOT_QUERY
  ).find(el => el.indexOf('user_robots_robots') === 0);
  if (isExistUserRobots) {
    const dataRobots = context.cache.readQuery({ query: USER_ROBOTS });
    context.cache.writeQuery({
      query: USER_ROBOTS,
      data: {
        robots: dataRobots.robots.filter(
          el => el.id !== variables.robot.userRobotId
        )
      }
    });
  }
...
export const actionRobot = (_root: any, variables: any, context: any) => {
  ...
  if (isExistRobotsStats) {
    ...
    const row_user_robots = row.user_robots.map(rowItem => ({
      ...
      started_at:
        variables.message === 'started' ? dayjs(new Date()).toISOString() : null
    }));
